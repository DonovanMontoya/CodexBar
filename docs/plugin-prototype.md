...
ClawRouter, Deepgram, sub2api, Synthetic, and Poe use the same bundled script on Apple platforms and Linux; their
...
Set `CODEXBAR_JS_PROVIDERS=1` in CodexBar's environment. OpenAI, z.ai, xAI, Manus,
...
and Poe always resolve only their script strategy on every platform; `CODEXBAR_JS_PROVIDERS` does not affect them.
