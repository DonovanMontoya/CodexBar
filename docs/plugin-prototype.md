...
ClawRouter, Deepgram, sub2api, Synthetic, Poe, and xAI use the same bundled script on Apple platforms and Linux; their
...
Set `CODEXBAR_JS_PROVIDERS=1` in CodexBar's environment. OpenAI, z.ai, Manus,
...
Poe, and xAI always resolve only their script strategy on every platform; `CODEXBAR_JS_PROVIDERS` does not affect them.
